...
def main():
    ...
    # --- AND THE NUMBERS ON THE TRANSLATED PAGES ------------------------------------------
    #
    # `_corpus` adds `site/index.html` with a sentence: "A claim is a claim wherever it is
    # published", written after that page was found carrying "576 ordinary support requests"
    # while the corpus held 1,440. Fifteen more pages are published from the same source and
    # none of them was in that corpus -- and they are the pages most of the people who read
    # them will read INSTEAD of the English one, so a number wrong there is wrong for the
    # whole of that audience.
    #
    # A translator may REFORMAT a number and must not CHANGE one. `1,500` is `1 500` in
    # Czech, French and Ukrainian and `1.500` in German, all of which are the same quantity,
    # so the separators come out before the comparison.
    #
    # TWO DIGITS OR MORE, deliberately. A single digit is prose that a language may spell out
    # -- Japanese writes English's "two models" as `モデル二つ`, and a language that wrote it
    # as `2` would fail a stricter rule for being correct. Every measured quantity this site
    # publishes has two digits or more.
    _SEPS = ",.\u00a0\u202f\u2009 "
    _num_run = re.compile("[0-9](?:[" + _SEPS + "]?[0-9])*")

    def _quantities(text):
        """Every number of two digits or more, separators normalised away."""
        out = []
        for _m in _num_run.findall(text or ""):
            _d = re.sub("[^0-9]", "", _m)
            if len(_d) >= 2:
                out.append(str(int(_d)))
        return sorted(out)

    check("the quantity reader sees a number at all",
          _quantities("1,500 probes across 30 targets") == ["30", "1500"] or
          _quantities("1,500 probes across 30 targets") == sorted(["1500", "30"]),
          str(_quantities("1,500 probes across 30 targets")))
    check("...and reads a separator as a separator, not as two numbers",
          _quantities("1 500") == _quantities("1,500") == _quantities("1.500") == ["1500"],
          "%s / %s / %s" % (_quantities("1 500"), _quantities("1,500"),
                            _quantities("1.500")))
    check("...and leaves single digits to prose",
          _quantities("3 tries apiece") == [], str(_quantities("3 tries apiece")))

    _off_by, _pairs_seen = [], 0
    for _lang in sorted(set(_langs) - {_i18n.DEFAULT}):
        _dict_p = os.path.join(ROOT, "site", "i18n", "%s.json" % _lang)
        if not os.path.exists(_dict_p):
            continue
        _strings = (json.loads(io.open(_dict_p, encoding="utf-8").read())
                    .get("strings") or {})
        for _src_s, _tr_s in _strings.items():
            if not _tr_s:
                continue
            _pairs_seen += 1
            if _quantities(_src_s) != _quantities(_tr_s):
                _off_by.append("%s: %r %s -> %s" % (_lang, _src_s[:40],
                                                    _quantities(_src_s),
                                                    _quantities(_tr_s)))
    check("there are translated strings to check at all", _pairs_seen > 100,
          "%d string(s) across %d language(s)" % (_pairs_seen, len(_langs) - 1))
    check("no translated string states a different number from the English",
          not _off_by, "; ".join(_off_by[:3]))

    # THE PAGE ITSELF IS ALREADY GATED, further down this file: "every published page is
    # exactly what the source generates", over the same `build(write=False)`. What was
    # missing is the half above -- the page can be exactly what its dictionary says while
    # the dictionary says the wrong number.
